...
class StaticBackgroundImage(QLabel):
    ...
    def __init__(self, parent=None, background_folder="src/shared/essentials/background"):
    ...
    def load_random_background(self):
        ...
        try:
            ...
            if not jpg_files:
                print(f"No JPG files found in {self.background_folder}")
                self.set_fallback_background()
                return
            
            # Select random image
            selected_image = random.choice(jpg_files)
            print(f"Selected background: {selected_image.name}")
            ...
            print(f"Background loaded: {selected_image.name} ({pixmap.width()}x{pixmap.height()})")
