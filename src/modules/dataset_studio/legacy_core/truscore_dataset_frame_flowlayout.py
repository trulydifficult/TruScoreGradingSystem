...
from modules.continuous_learning.guru_dispatcher import get_global_guru
...
class ConversionWorker(QThread):
    ...
    def run(self):
        try:
            # Import converter
            from .yolo_to_maskrcnn_converter import YOLOToMaskRCNNConverter
            ...
            # Emit success with minimal data - don't pass huge conversion_result across threads!
            result_data = {
            ...
            # Don't pass conversion_result - it's massive and blocks thread communication
...
class TruScoreDatasetFrame(QFrame):
    ...
    def setup_flow_grid(self, layout):
        ...
        self.flow_container.setStyleSheet("QWidget { background-color: #1a1a1a; }")
    ...
    def setup_image_preview_panel(self, layout):
        ...
        """)
        preview_frame.setFixedWidth(420)  # Increased from 320 to accommodate larger preview
        ...
        """)
        self.preview_display_frame.setFixedSize(400, 550)  # Increased from 300x420
    ...
    def create_labels_tab(self):
        ...
        # Create sub-tab widget for YOLO/COCO labels
    ...
    def create_source_labels_subtab(self):
        ...
        self.import_labels_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {TruScoreTheme.QUANTUM_GREEN};
                color: white;
                border: 3px solid {TruScoreTheme.NEON_CYAN};
        ...
        self.clear_labels_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {TruScoreTheme.ERROR_RED};
                color: white;
                border: 3px solid {TruScoreTheme.PLASMA_BLUE};
        ...
        # Labels list - single column grid for visual inspection
        ...
        self.labels_container.setStyleSheet(f"QWidget {{ background-color: {TruScoreTheme.NEURAL_GRAY}; }}")
    ...
    def create_verification_tab(self):
        ...
        # Left side - Verification grid area  
        left_frame = QFrame()
        left_frame.setStyleSheet(f"""
            QFrame {{
                background-color: {TruScoreTheme.QUANTUM_DARK};
    ...
    def populate_verification_tab(self):
        ...
        self.verification_images_to_load = list(self.images)
    ...
    def _image_has_label(self, image_path):
        ...
        # 🚨 PRIORITY 1: Check COCO annotations first (most common for loaded projects)
        if hasattr(self, 'coco_annotations') and self.coco_annotations:
            ...
            return False  # Image not in COCO data
        
        # 🚨 PRIORITY 2: Check individual label files (YOLO format, etc.)
    ...
    def setup_verification_flow_grid(self, layout):
        ...
        self.verification_flow_container.setStyleSheet("QWidget { background-color: #1a1a1a; }")
    ...
    def setup_verification_preview_panel(self, layout):
        """Setup verification preview panel - shows image + label overlay (ENLARGED)"""
        preview_frame = QFrame()
        preview_frame.setStyleSheet(f"""
            QFrame {{
                background-color: {TruScoreTheme.NEURAL_GRAY};
        ...
        """)
        preview_frame.setFixedWidth(420)  # Increased from 320 to match Images tab
        layout.addWidget(preview_frame)
        ...
        # Preview display area (ENLARGED)
        self.verification_preview_frame = QFrame()
        self.verification_preview_frame.setStyleSheet(f"""
            QFrame {{
                background-color: {TruScoreTheme.QUANTUM_DARK};
                border-radius: 8px;
                border: 2px solid {TruScoreTheme.PLASMA_BLUE};
        ...
        """)
        self.verification_preview_frame.setFixedSize(400, 550)  # Increased from 300x420 to match Images tab
    ...
    def create_analysis_dashboard(self, layout):
        ...
        # Quality Analysis (MORE PROMINENT HEADER)
        quality_label = QLabel("Quality Analysis")
        quality_label.setFont(QFont("Permanent Marker", 16))  # Increased from 10 to 16
        quality_label.setStyleSheet(f"color: {TruScoreTheme.QUANTUM_GREEN}; padding: 10px 0px; font-weight: bold;")
        quality_label.setFixedHeight(45)
        ...
        stats_layout.addWidget(self.quality_frame)
    ...
    def load_images_into_flow_layout(self, image_paths):
        ...
        try:
            ...
            print(f"Error starting progressive loading: {e}")
    ...
    def load_next_image(self):
        ...
        try:
            if not hasattr(self, 'loading_queue') or not self.loading_queue:
                ...
                print(f"Completed: {total_loaded} images loaded")
                self.update_status(f"Loaded {total_loaded} images successfully")
                
                # 🚨 FIX: Update analysis dashboard after all images are loaded
                self.update_analysis_dashboard()
            ...
            # Update status every 25 images to reduce CLI spam
    ...
    def _fallback_label_dialog(self):
        ...
        try:
            ...
            print(f"Fallback label dialog failed: {e}")
    ...
    def _process_label_files(self, file_paths):
        ...
        try:
            ...
            try:
                ...
                print("Warning: Label-pipeline compatibility checker not available")
            
            # Silent processing - no CLI spam
            ...
            for file_path in file_paths:
                ...
                if extension in supported_formats:
                    ...
                    # 🚨 CRITICAL: Check pipeline compatibility if available
            ...
            # 🚨 CRITICAL: Show compatibility report
    ...
    def _convert_yolo_to_coco(self, pipeline_name: str):
        ...
        try:
            from src.core.dataset_creator.yolo_to_maskrcnn_converter import YOLOToMaskRCNNConverter
            ...
            if hasattr(self, 'images') and hasattr(self, 'imported_labels') and self.images and self.imported_labels:
                ...
                # Convert to image-path based mapping that the converter expects
                print(f"DEBUG: Processing {len(self.imported_labels)} label files...")
            ...
            QMessageBox.critical(
                self,
                "Conversion Failed",
                f"Label conversion failed: {str(e)}\\n\\nPlease check your label files and try again."
            )
...
Path: {label_info['path']}
    ...
    def on_conversion_completed(self, result_data):
        ...
        try:
            self.logger.debug("Callback received, updating UI...")
            ...
            self.logger.debug("Extracted results, updating COCO tab...")
            
            # 🚨 CRITICAL: Update COCO Labels tab with lightweight approach
            if hasattr(self, 'update_coco_labels_tab'):
                # Load conversion result from file instead of passing massive data across threads
                try:
                    import json
                    ...
                    print(f"Warning: Could not load conversion result for COCO tab: {e}")
                    # Fallback: just update status without full COCO tab update
                    pass
            
            self.logger.debug("COCO tab updated, switching tabs...")
    ...
    def update_quality_analysis(self, high_quality_count, total_count):
        ...
        try:
            ...
            if hasattr(self, 'quality_frame') and self.quality_frame.layout():
                for i in reversed(range(self.quality_frame.layout().count())):
                    child = self.quality_frame.layout().itemAt(i).widget()
                    if child:
                        child.setParent(None)
            elif not hasattr(self, 'quality_frame'):
                self.logger.warning("quality_frame not available for quality analysis update")
                return
...
Proceed with export?
    ...
    def _generate_photometric_data(self, images_to_export, dataset_export_dir):
        ...
        try:
            from src.core.truscore_system.photometric.photometric_stereo import TruScorePhotometricStereo
            import cv2
