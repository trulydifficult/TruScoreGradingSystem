...
def _sanitize_value(value):
    ...
    # Check for methods like x(), y() without importing PyQt to keep server light
...
def _detailed_snapshot(result: Any) -> Dict[str, Any]:
    ...
    if isinstance(corners, dict):
        # CLEANUP: Remove raw image arrays (crops) from the text JSON to prevent crashes/bloat
        # We save these as actual image files in _serialize_visualizations instead
    ...
    insights = getattr(result, "visualization_data", {}).get("insights")
    if insights:
        details["insights"] = _sanitize_value(insights)

    # Pass through smart defects
    defects = getattr(result, "visualization_data", {}).get("smart_defects")
    if defects:
        details["smart_defects"] = _sanitize_value(defects)
        details["defects"] = details["smart_defects"]
...
def _serialize_visualizations(result: Any, job_id: str, side: str, image_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    # 1. Photometric stereo maps (surface normals, depth, albedo, defects, confidence)
        ...
        if normals is not None:
            ...
            if norm_img is not None:
                ...
                out["assets"].append({
                    ...
                    "label": "Surface Normals (3D)",
                    "url": f"/api/v1/cards/{job_id}/viz/{dest.name}"
                })
    ...
    # 2. Centering overlay (rendered even if no pixmap was preserved)
    ...
    if isinstance(centering, dict):
        ...
        # Sanitize metadata (remove Pixmaps or objects)
    ...
    if isinstance(corners_data, dict):
        ...
        # Sanitize metadata (remove crops array)
    ...
    # 4. Generic catch-all for any other visualization payloads
